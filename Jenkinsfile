pipeline {
    ...
    stages { 
        ...
        stage('Run Images'){
            steps {
                sh '''sudo docker rm -f flask-simple-webservice || true && sudo docker pull linothomas/flask-simple-webservice:latest && sudo docker run -d -p 80:5000 --name flask-simple-webservice linothomas/flask-simple-webservice:latest'''
            }
        }
}
}